...
app.get(
    ...
    async (req, res) => {
        ...
        const clientServiceUrl = process.env.CLIENT_SERVICE_URL || "http://localhost:8081"; // Note: Client Service
        const accountServiceUrl = process.env.ACCOUNT_SERVICE_URL || "http://localhost:8082"; //porta do account-service
        ...
        try {
            ...
            const clientPromise = axios.get(`${clientServiceUrl}/clientes/${cpf}`, config);

            // Chamada 2: Account Query Service (GET /query/my-account)
            const accountPromise = axios.get(`${accountServiceUrl}/query/my-account`, config);
