...
const authorizeClientesList = (req, res, next) => {
  ...
  if (tipo === "GERENTE" || tipo === "MANAGER" || tipo === "ADMINISTRADOR")
    return next();
  return res.status(403).json({ error: "Forbidden" });
};
...
const authorizeClienteByCpf =
  ...
  (req, res, next) => {
    ...
    if (tipo === "ADMINISTRADOR" || tipo === "GERENTE" || tipo === "MANAGER")
      return next();
    ...
    try {
      ...
      try {
        ...
        if (accountError.response?.status !== 404) {
          console.warn(
            `Erro ao buscar conta para CPF ${cpf}:`,
...
app.get(
  ...
  async (req, res, next) => {
    ...
    if (
      ...
      try {
        console.log(
          `[Gateway] Agregando dados para ${userRole}, filtro: ${filtro}`
        );
        ...
        let relatorio = clients.map((client) => {
          ...
          return {
            ...
            limite:
              conta && conta.limit != null ? conta.limit : client.limite || 0,
        ...
        return res
          .status(503)
          .json({ error: "Erro ao agregar dados de clientes." });
      }
    }
...
app.put(
  ...
  proxy(services.client, {
    ...
    proxyErrorHandler: (err, res, next) => {
      res
        .status(503)
        .json({ error: "Client Service indisponível para atualização." });
    },
  })
);
...
app.post(
  ...
  proxy(services.saga, {
    ...
    proxyErrorHandler: (err, res, next) => {
      res
        .status(503)
        .json({ error: "Saga Orchestrator indisponível para aprovação." });
    },
  })
);
...
app.post(
  ...
  proxy(process.env.SAGA_ORCHESTRATOR_URL || "http://localhost:8085", {
    ...
    proxyErrorHandler: (err, res, next) => {
      ...
      res
        .status(503)
        .json({ error: "Saga Orchestrator unavailable", message: err.message });
    },
  })
);
      ...
      if (dia.movimentacoes && Array.isArray(dia.movimentacoes)) {
        const movs = dia.movimentacoes.map((m) => {
          ...
          if (tipo.includes("deposit") || tipo.includes("deposito"))
            tipo = "depósito";
          else if (tipo.includes("withdraw") || tipo.includes("saque"))
            tipo = "saque";
...
app.post(
  ...
  proxy(process.env.AUTH_SERVICE_URL || "http://localhost:8084", {
    proxyReqPathResolver: (req) => {
      console.log(
        `📤 [LOGOUT] ${req.method} ${req.originalUrl} - User: ${req.user?.cpf}`
      );
...
app.get("/reboot", async (req, res) => {
  ...
  res.status(statusCode).json({
    ...
    summary: hasError
      ...
      : "Todos os serviços resetados com sucesso",
    results: results,
  });
});
...
app.listen(PORT, () => {
  ...
  console.log(
    `   • Saga Service:    ${
      process.env.SAGA_ORCHESTRATOR_URL || "http://localhost:8085"
    }`
  );
  console.log(
    `   • Auth Service:    ${
