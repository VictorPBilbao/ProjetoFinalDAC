...
export class ClientService {
  ...
  addClient(newClient: Omit<Cliente, 'id' | 'criadoEm'>): Observable<any> {
    ...
    return of({ success: true, message: 'Cliente criado com sucesso!' }).pipe(delay(500));
  }

  updateClient(updatedClient: Cliente): Observable<any> {
    this.storage.updateCliente(updatedClient);
    return of({ success: true, message: 'Cliente atualizado com sucesso!' }).pipe(delay(500));
  }

  deleteClient(clientId: string): Observable<any> {
    this.storage.deleteCliente(clientId);
    return of({ success: true, message: 'Cliente removido com sucesso!' }).pipe(delay(500));
  }
}
